...
def _spatial_average(
  ...
  dims_to_sum = [dim for dim in ['latitude', 'longitude'] if dim in weights.dims]
  print("Default weights sum:", weights.sum(dim=dims_to_sum).values)
  if region is not None:
    print("region is not None")
    print("region: ", region)
    print(f"Region class: {type(region).__name__}")
    dataset, weights = region.apply(dataset, weights)
    print("region.apply weights: ", weights)
    # ignore NaN/Inf values in regions with zero weight
    dataset = dataset.where(weights > 0, 0)
  print("Weights sum before average:", weights.sum().item())
  print("Weights NaNs:", weights.isnull().any())
  print("Weights == 0:", (weights == 0).sum())
...
class MSE(Metric):
  ...
  def compute_chunk(
    ...
    print("MSE with lat", forecast.latitude)
    print(results)
    return results
...
class ACC(Metric):
  ...
  def compute_chunk(
    ...
    print("Initial forecast latitudes:", forecast.latitude.values)
    print("Initial climatology_chunk latitudes:", climatology_chunk.latitude.values)
    print("Intersecting lats with forecast:",
      np.intersect1d(forecast.latitude.values, climatology_chunk.latitude.values))
    print("Intersecting lats with truth:",
      np.intersect1d(truth.latitude.values, climatology_chunk.latitude.values))
    print("Intersecting lats: truth and forecast:",
      np.intersect1d(forecast.latitude.values, truth.latitude.values))
    # climatology_chunk = climatology_chunk.interp_like(forecast)
    # forecast_anom = forecast - climatology_chunk
    # truth = truth.interp_like(forecast)
    # climatology_chunk = climatology_chunk.interp_like(truth)
    # truth_anom = truth - climatology_chunk
    
    # Align climatology to forecast grid
    # climatology_on_forecast = climatology_chunk.interp_like(forecast, method='nearest')
    climatology_on_forecast = climatology_chunk.reindex_like(forecast, method="nearest", tolerance=1e-3)
    # climatology_chunk = climatology_chunk.interp_like(forecast)
    forecast_anom = forecast - climatology_on_forecast

    lon_diff = np.setdiff1d(forecast.longitude.values, climatology_chunk.longitude.values)
    print("Longitudes in forecast but not in climatology:", lon_diff)
    # Align truth to forecast grid
    # truth_on_forecast = truth.interp_like(forecast)
    truth_anom = truth - climatology_on_forecast
    print("Initial forecast_anom latitudes:", forecast_anom.latitude.values)
    print("Initial truth_anom latitudes:", truth_anom.latitude.values)

    # for dim in ["init_time", "lead_time", "latitude", "longitude"]:
    #   nan_frac = forecast_anom["tas"].isnull().mean(dim)
    #   print(f"NaN fraction along {dim}:", nan_frac)

    print("forecast_anom NaNs:", forecast_anom.isnull().any())
    print("truth_anom NaNs:", truth_anom.isnull().any())
...
class CRPS(EnsembleMetric):
  ...
  def compute_chunk(
    ...
    return_val= CRPSSkill(self.ensemble_dim).compute_chunk(
        forecast,
        truth,
        region=region,
        skipna=skipna,
    ...
    print(return_val.isnull().any(),"NaNs check compute_chunk (694)!")
    return return_val
...
def _pointwise_crps_spread(
  ...
  print(forecast.isnull().any(), "NaNs check forecast!")
  print(return_val.isnull().any(), "NaNs check in return of _pointwise_crps_spread!")
  return return_val
...
def _pointwise_crps_skill(
  ...
  print("Difference in latitudes:", np.abs(forecast['latitude'].values - truth['latitude'].values))
  print("Difference in longitudes:", np.abs(forecast['longitude'].values - truth['longitude'].values))
  truth = truth.interp(latitude=forecast.latitude, longitude=forecast.longitude)

  return_val = abs(truth - forecast).mean(ensemble_dim, skipna=skipna)

  print(forecast.isnull().any(), "NaNs check in forecast!")
  print(truth.isnull().any(), "NaNs check in truth!")
  print(return_val.isnull().any(), "NaNs check in return of _pointwise_crps_skill!")
  return return_val
...
def _compute_brier_score(
  ...
  # probabilities).
  threshold = threshold.reindex_like(forecast, method="nearest", tolerance=1e-3)
  lon_diff = np.setdiff1d(forecast.longitude.values, threshold.longitude.values)
  print("Longitudes in forecast but not in climatology threshold:", lon_diff)
