...
class SliceRegion(Region):
  ...
  def apply(  # pytype: disable=signature-mismatch
    ...
    print("Original lat_slice:", self.lat_slice)
    print("Original lon_slice:", self.lon_slice)
    ...
    print(f"Concatenated latitudes shape: {lats.shape}, values: {lats.values}")
    print(f"Concatenated longitudes shape: {lons.shape}, values: {lons.values}")
    ...
    sliced_dataset = dataset.sel(latitude=lats, longitude=lons)
    sliced_weights = weights.sel(weight_indexers)

    print(f"Sliced dataset dims: {sliced_dataset.dims}, sizes: { {k: v for k,v in sliced_dataset.sizes.items()} }")
    print(f"Sliced weights dims: {sliced_weights.dims}, sizes: { {k: v for k,v in sliced_weights.sizes.items()} }")
    print(f"Sliced weights sum: {sliced_weights.sum().values}")
