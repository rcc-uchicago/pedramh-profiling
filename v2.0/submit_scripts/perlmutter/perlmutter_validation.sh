...
if [[ "$3" == "1" ]]; then
	CMD="python train.py --yaml_config=${2} --run_num=${1} --debug --just_validate --validation_epochs=$4"
else
	CMD="python -m torch.distributed.launch --nproc_per_node=${NUM_TASKS_PER_NODE} train.py --yaml_config=${2} --run_num=${1} --just_validate --validation_epochs=$4"
fi
