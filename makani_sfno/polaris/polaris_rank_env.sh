...
#
# VENDORED from physicsnemo_ai_rossby/polaris/polaris_rank_env.sh, executable
# body byte-identical.  It is duplicated rather than shared because that file
# lives in a different git subtree and the two projects must be launchable
# independently; if you change the body, change BOTH.
#
# It applies unchanged to Makani, which is not obvious from either directory:
# makani/utils/comm.py:19 imports physicsnemo's DistributedManager and
# comm.init() calls DistributedManager.initialize(), so a makani job inherits
# exactly physicsnemo's launcher detection (physicsnemo/distributed/manager.py):
...
#       bash polaris/polaris_rank_env.sh python -m sfno_training.train_plasim ...
#
