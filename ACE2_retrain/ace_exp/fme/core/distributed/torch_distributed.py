    ...
    def wrap_module(self, module: torch.nn.Module) -> torch.nn.Module:
        if any(p.requires_grad for p in module.parameters()):
            ...
            return DistributedDataParallel(
                ...
                # write grads into bucket views: less memory, better overlap
                gradient_as_bucket_view=True,
                # no per-step buffer broadcast. Safe while the model uses
                # instance_norm (no running stats) and the SHT quadrature /
                # Legendre buffers are identical on every rank by
                # construction. Drop this if a BatchNorm layer is added.
