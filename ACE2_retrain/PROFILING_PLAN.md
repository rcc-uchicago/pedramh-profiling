...
| *"NCCL kernel time is not comm cost… treat it as an **upper bound**"* — caveat never resolved | **NOT straggler-bound IN THAT CAPTURE — demoted 2026-08-20 from "resolved".**
  Balance is a per-run draw, not a property: `polaris_bench_report.md` §4.4 finds **1 vs 17
  stalled steps of 40** across two runs of an identical config, and NCCL +114.8%. Re-check
  per-rank balance in every capture (`nvtx_phase_attribution.py --per-rank`, rooted
  collectives excluded) instead of treating this as closed. Original finding: 74% of NCCL time is *exposed*, not spin-wait: per-device total kernel time is 10.232/10.453/10.446/10.444 s (<2% rank imbalance), so it is not straggler wait. 35.7% of wall-clock is all-reduce with an otherwise-idle SM array. | Already settled. Confirm by a second method with `ddp._get_ddp_logging_data()` (`avg_backward_comm_time` − `avg_backward_compute_comm_overlap_time`). |
...
- ⚠ **Second candidate, added 2026-08-20 — and it is structural, not Pangu-specific.**
  `polaris_bench_report.md` §4.5 attributes **49% of Pangu's copy time** to materialising the
  *permuted* 377 MB `dhconv` spectral weight for `einsum("bixy,iox->boxy")` — which also
  reports as `aten::clone`, so the ACE2 measurement **cannot distinguish it** from the
  autocast-boundary story below. ACE2 has the same weight class: `operator_type: dhconv`,
  `filter_type: linear`, `embed_dim: 384`, `num_layers: 8`, `hard_thresholding_fraction: 1.0`
  ⇒ a `[in, out, modes_lat]` complex weight at **384×384×180 = 212.34 MB/layer**, ~**93%** of
  ACE2's parameters, and its contraction slices the weight before contracting. **It has never
  been checked there.** The census separates the two candidates for free, by shape:
  **26,542,080 complex elements = weight; 12,510,720 = activation.** Cheapest first check,
  no job, no GPU: `python3 ACE2_retrain/sfno_bytes_model.py --match <ace2.sqlite>` with
...
| **`conj_kernel_cuda`** 1.51% ⚠ **on Pangu this is 14.1% of copy time = 38.30 ms/rank-step = 6.3% of the step, not 1.5%** (`polaris_bench_report.md` §4.5b — the conjugated tensor is the 377 MB spectral weight in the einsum's permuted layout); size it per model. This row's gate assessment ("plausibly bit-exact") is the right one, and §4.5c cites it rather than re-deriving it. — PyTorch has had a lazy conjugate bit since 1.10; something in the `view_as_complex`/`view_as_real` round trip is materialising it | 1.5% | plausibly **bit-exact** (a conj view and its materialisation hold identical values) → the cheapest gated item. May live in vendored `torch_harmonics 0.8.0`. |
