...
class PanguModelMemEff(nn.Module):
    ...
    def __init__(self, params, embed_dim=192, num_heads=(6, 12, 12, 6), window_size=(2, 6, 12)):
        ...
        self.layer1 = EarthSpecificLayerMemEff(
            dim=embed_dim,
            input_resolution=(8, 181//params.img_scale, 360//params.img_scale),
            depth=2,
            num_heads=num_heads[0],
        ...
        self.downsample = DownSample(in_dim=embed_dim, input_resolution=(8, 181//params.img_scale, 360//params.img_scale),
                                     output_resolution=(8, 91//params.img_scale, 180//params.img_scale))
        self.layer2 = EarthSpecificLayerMemEff(
            dim=embed_dim * 2,
            input_resolution=(8, 91//params.img_scale, 180//params.img_scale),
            depth=6,
            num_heads=num_heads[1],
        ...
        self.layer3 = EarthSpecificLayerMemEff(
            dim=embed_dim * 2,
            input_resolution=(8, 91//params.img_scale, 180//params.img_scale),
            depth=6,
            num_heads=num_heads[2],
        ...
        self.upsample = UpSample(embed_dim * 2, embed_dim, (8, 91//params.img_scale, 180//params.img_scale),
                                 (8, 181//params.img_scale, 360//params.img_scale))
        self.layer4 = EarthSpecificLayerMemEff(
            dim=embed_dim,
            input_resolution=(8, 181//params.img_scale, 360//params.img_scale),
            depth=2,
            num_heads=num_heads[3],
...
class EarthSpecificLayerMemEff(nn.Module): #BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, mlp_ratio=4., qkv_bias=True, qk_scale=None,
        ...
        self.blocks = nn.ModuleList([
            EarthSpecificBlockMemEff(dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size,
                               shift_size=(0, 0, 0) if i % 2 == 0 else None, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                               qk_scale=qk_scale, drop=drop, attn_drop=attn_drop,
                               drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                               norm_layer=norm_layer)
...
class EarthSpecificBlockMemEff(nn.Module):
    """
    3D Transformer Block
    Args:
        dim (int): Number of input channels.
...
class EarthAttention3DMemEff(nn.Module):
    """
    3D window attention with earth position bias.
    It supports both of shifted and non-shifted window.
    ...
    def forward(self, x: torch.Tensor, mask=None):
        ...
        print(q.shape)
        print(earth_position_bias[:, :, :, :L].shape)
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_mem_efficient=True, enable_math=False):
            if mask is not None:
                nLon = mask.shape[0]
                x = F.scaled_dot_produce_attention(q.view(B_ * self.num_heads, nW_, N, L),
                                                    k.view(B_ * self.num_heads, nW_, N, L),
                                                    v.view(B_ * self.num_heads, nW_, N, L),
                                                    attn_mask=earth_position_bias[:, :, :, :L] + \
                                                            mask.unsqueeze(1).expand(nLon, B_ * self.heads // nLon,nW_, N, N).reshape(-1, nW_, N, N)[:, :, :, :L],
                                                    scale = self.scale)
                x = x.view(-1, self.num_heads, nW_, N, L)
            else:
                x = F.scaled_dot_product_attention(q.view(B_ * self.num_heads, nW_, N, L),
                                                    k.view(B_ * self.num_heads, nW_, N, L),
                                                    v.view(B_ * self.num_heads, nW_, N, L),
                                                   attn_mask=earth_position_bias[:, :, :, :L], scale=self.scale)
        """
