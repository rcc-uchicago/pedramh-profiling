...
#SBATCH --exclusive
#SBATCH --mem=500G
#SBATCH -o snfo_bench_%x_%j.out
#SBATCH -e snfo_bench_%x_%j.err

# Wall-clock throughput benchmark for SNFO training.
...
#   1. Set SNFO_NVTX=1 below and raise SNFO_BENCH_WARMUP to 40.
#   2. Wrap the srun call with:
#        nsys profile \
#          --trace=cuda,nvtx,osrt \
#          --capture-range=cudaProfilerApi \
#          --output=snfo_bench_%j \
#          srun python bench.py ...
#   3. After the job: nsys export --type=sqlite snfo_bench_<job>.nsys-rep
...
# bench.py reads SNFO_PRECISION and applies it on top of the YAML.
export SNFO_PRECISION=bf16-mixed
# export SNFO_PRECISION=32-true   # uncomment for an fp32 baseline (lower bs=2 first)
# export SNFO_PRECISION=16-mixed

# NVTX: set to 1 together with the nsys wrapper above.
# export SNFO_NVTX=1
# export SNFO_BENCH_WARMUP=40   # raise warmup when using torch.compile

# CSV output: absolute path so it survives cwd changes.
export SNFO_BENCH_CSV="${SLURM_SUBMIT_DIR}/bench_pedramh_node_results.csv"

config_file=configs/bench_midway.yaml

echo "=== snfo_bench: $(date -Iseconds) ==="
