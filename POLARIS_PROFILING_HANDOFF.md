...
| "NCCL kernel time is an **upper bound** on comm cost" | **RESOLVED, and worse than the caveat.** Per-device kernel totals differ by <2.2% (10.232/10.453/10.446/10.444 s), so it is not straggler wait. On **4× H100 NVL at batch 4**, **35.7% of wall-clock is *exposed* all-reduce** and only 26.3% of NCCL overlaps compute (job 53534648; 33.2–37.7% and 25.4–30.6% across the four ranks). **Per-node and per-batch, not an SFNO constant** — the range across captures on disk is 44% → 30% → 7% of GPU-busy time; see §2. |
