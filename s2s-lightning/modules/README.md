...
Mirrors `$SI_DIR/modules/` (`/project/pedramh/shared/anthonyz/modules/`) for the
...
- `train_module.py` — **landed.** `TrainModule(L.LightningModule)`, mirrors
  `$SI_DIR/modules/train_module.py`. Holds S2S's `PanguModel_Plasim`
  ...
  Trainer).
- `ae_module.py` / `combined_module.py` — **not ported** (SI-specific
  autoencoder / evaluation modules; S2S has no analogue).
- `models/` — intentional **empty placeholder** (this README + `models/README.md`
  only). SI keeps its model definitions under `modules/models/` (`DiT.py`,
...
SI source packages have **no `__init__.py`** (PEP 420 namespace packages,
