...
Mirrors `$SI_DIR/configs/` (`/project/pedramh/shared/anthonyz/configs/`), which
holds plain YAML files loaded with `yaml.safe_load` (via `common/utils.py::get_yaml`)
plus an archived `configs/old/`. SI YAML has three top-level keys — `model:`,
...
adopt SI's flat `get_yaml` + `model`/`data`/`training` split — a necessary-S2S
