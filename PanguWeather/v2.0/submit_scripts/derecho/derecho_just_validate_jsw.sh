...
#PBS -l select=1:ncpus=32:ngpus=4:mem=480G
#PBS -q develop
...
if [[ "$DEBUG" == "1" ]]; then
	CMD="python train.py --yaml_config=${YAML_CONFIG} --run_num=${RUN_NUM} --debug --config=SFNO --just_validate --validation_epochs="30,40,50,57,62""
else
	CMD="python -m torch.distributed.launch --nproc_per_node=${NUM_TASKS_PER_NODE} train.py --yaml_config=${YAML_CONFIG} --run_num=${RUN_NUM} --config=SFNO --just_validate --validation_epochs="30,40,50,57,62""
fi
# if [[ -z "$JOBID" ]]; then
# 	CMD+=" --fresh_start"
# fi
