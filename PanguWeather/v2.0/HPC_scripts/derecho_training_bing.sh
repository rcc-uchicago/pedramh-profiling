...
#PBS -l select=1:ncpus=64:ngpus=4:mem=480G
#PBS -q develop
...
module load conda
conda activate /glade/work/jesswan/conda-envs/sfno_pangu
#
...
YAML_CONFIG="/glade/work/bgong/Pangu_jess/v2.0/config/E3SM_SFNO_H5_DERECHO_jsw.yaml"
RUN_NUM="sfno_pangu_test_bing"

if [[ "$DEBUG" == "1" ]]; then
	CMD="python train_optimized.py --yaml_config=${YAML_CONFIG} --run_num=${RUN_NUM} --debug --config=SFNO"
else
	CMD="python -m torch.distributed.launch --nproc_per_node=${NUM_TASKS_PER_NODE} train_optimized.py --yaml_config=${YAML_CONFIG} --run_num=${RUN_NUM} --config=SFNO"
fi
# if [[ -z "$JOBID" ]]; then
# 	CMD+=" --fresh_start"
# fi
