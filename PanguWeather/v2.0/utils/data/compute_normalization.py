...
def main():
    ...
    if lev_type == 'plev':
        ...
        test_ds = open_dataset(os.path.join(root_dir, list_pressure_vars[0], f'{start_year}_gaussian.nc'))
        ...
        test_ds = open_dataset(os.path.join(root_dir, f'{list_pressure_vars[0]}_sigma', f'{start_year}_gaussian.nc'))
        levels = test_ds[lev_type].values
        test_plev_ds = open_dataset(os.path.join(root_dir, list_pressure_vars[0], f'{start_year}_gaussian.nc'))
    ...
    for var in tqdm(list_single_vars + list_pressure_vars, desc='variables', position=0):
        ...
        for year in tqdm(years, desc='years', position=1, leave=False):
            ...
            if var in SINGLE_LEVEL_VARS:
                path = os.path.join(root_dir, var, f'{year}_gaussian.nc')
            
            else:
                if lev_type == 'plev' or var == 'zg':
                    path = os.path.join(root_dir, var, f'{year}_gaussian.nc')
                else:
                    path = os.path.join(root_dir, f'{var}_sigma', f'{year}_gaussian.nc')
    ...
    for var in list_constant_vars + list_varying_boundary:
        if steps is not None:
            ...
            if var in list_varying_boundary:
                path = os.path.join(root_dir, f'{var}_masked_6h.nc')
            else:
                path = os.path.join(root_dir, f'{var}_masked.nc')
            ds = open_dataset(path)
            ds_np = ds[var].values
            ds_np = np.where(np.isnan(ds_np), mask_fill[var], ds_np)
